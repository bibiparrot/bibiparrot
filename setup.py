...
###############################################################################
# Name: setup.py                                                              #
# Purpose: Setup/build script for Bibiparrot                                   #
# Author: Chunqi SHI <diligence.cs@gmail.com>                                 #
# Copyright: (c) 2013-2014 Chunqi SHI <diligence.cs@gmail.com>                #
# License: wxWindows License                                                  #
###############################################################################

"""
 Bibiparrot Setup Script
 ...
 @summary: Used for building the Bibiparrot distribution files and installations
...
#---- Global Settings ----#
APP = ['src/BibiParrot.py']
AUTHOR = "Chunqi SHI"
AUTHOR_EMAIL = "diligence.cs@gmail.com"
YEAR = 2013
...
def GenerateBinPackageFiles():
    ...
    pixlist = ["pixmaps/Bibiparrot.png", "pixmaps/Bibiparrot_doc.png"]

    if "darwin" in sys.platform:
        data.append("pixmaps/Bibiparrot_doc.icns")
        pixlist.extend(["pixmaps/Bibiparrot.icns", "pixmaps/Bibiparrot_doc.icns"])
    elif sys.platform.startswith("win"):
        data.extend(glob.glob("include/windows/*.*"))
        pixlist.append("pixmaps/Bibiparrot.ico")
...
def GenerateSrcPackageFiles():
    ...
    for loc_dir in os.listdir("locale"):
        tmp = "locale/" + loc_dir
        if os.path.isdir(tmp):
            tmp = tmp + "/LC_MESSAGES/Bibiparrot.mo"
...
Overview
========
Bibiparrot is a multi-platform text editor with an implementation that focuses on
...
screenshots visit the projects homepage at `Bibiparrot.org
<http://www.Bibiparrot.org/>`_.
...
ICON = { 'Win' : "pixmaps/Bibiparrot.ico",
         'WinDoc' : "pixmaps/Bibiparrot_doc.ico",
         'Mac' : "pixmaps/Bibiparrot.icns"
}
...
def BuildPy2Exe():
    ...
    setup(
        ...
        options = {"py2exe" : {"compressed" : 1,
                               ...
                               "dll_excludes": [ "MSVCP90.dll",
                                                 "tk85.dll",
                                                 "tcl85.dll" ] }},
        windows = [{"script": "src/Bibiparrot.py",
    ...
    shutil.copy2(".\\Bibiparrot-installer.nsi", ".\\dist\\Bibiparrot-installer.nsi")
...
def BuildOSXApp():
    ...
    PLIST = dict(CFBundleName = info.PROG_NAME,
             CFBundleIconFile = 'Bibiparrot.icns',
             ...
             CFBundleIdentifier = "org.Bibiparrot.%s" % info.PROG_NAME.title(),
             CFBundleDocumentTypes = [dict(CFBundleTypeExtensions=fextents,
                                           CFBundleTypeIconFile='Bibiparrot_doc',
...
#                                      'uk_UA', 'zh_CN'],
    ...
    DATA_FILES = GenerateBinPackageFiles()
    DATA_FILES.append("scripts/Bibiparrotmac.sh")
...
def CreateDMG(version):
    ...
    os.chdir('dist')
    vname = "Bibiparrot-%s" % version
    fname = vname + ".dmg"
    mpath = "/Volumes/Bibiparrot-%s" % version
    comp = "Bibiparrot-%s_2.dmg" % version
    ...
    os.system("ditto -rsrcFork Bibiparrot.app %s/Bibiparrot.app" % mpath)
...
tell application "Finder"
    tell disk ("%s" as string)
        ...
        set position of item "Bibiparrot.app" to {260, 145}
...
def DoSourcePackage():
    ...
    if '--no-clean' not in sys.argv:
        try:
            import Bibiparrot
            path = Bibiparrot.__file__
            if '__init__' in path:
                path = os.path.dirname(path)
                path = os.path.join(path, 'src')
                del sys.modules['Bibiparrot']
                shutil.rmtree(path)
    ...
    setup(
        ...
        maintainer_email = AUTHOR_EMAIL,
        url = URL,
        download_url = "http://Bibiparrot.org/download",
        license = LICENSE,
        platforms = [ "Many" ],
        packages = [ NAME ],
        package_dir = { NAME : '.' },
...
def BuildECLibDemo():
    """Build the Bibiparrot Control Library Demo package"""
